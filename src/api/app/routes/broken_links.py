...
router = APIRouter()
# Mounted first in main.py (before other /findings routers) so this path is never captured by /broken-links/{finding_id}.
stats_router = APIRouter()


@stats_router.get("/broken-links/stats", response_model=Dict[str, Any])
async def get_broken_links_stats(
    ...
    try:
        ...
        if program_name:
            ...
            stats = await BrokenLinksRepository.get_broken_links_stats()
        else:
            acc_list = list(accessible or [])
            stats = await BrokenLinksRepository.get_broken_links_stats(
                None, acc_list if acc_list else []
            )
