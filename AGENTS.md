...
Runner `Task.input_type` and `Task.output_types` are the source of truth for each task's inputs and outputs. They are mirrored as `input_types:` / `output_types:` arrays in [`src/api/app/recon_task_builtin_defaults.yaml`](src/api/app/recon_task_builtin_defaults.yaml) and exposed to clients (frontend, runner, tests) via the nested `GET /admin/public/recon-tasks/effective-parameters` manifest (`{parameters, input_types, output_types}` per task). The frontend workflow builder consumes this manifest via `src/frontend/src/stores/taskManifestStore.js`—not hardcoded constants. The runner asserts Python ↔ manifest agreement at startup (`TaskParameterManager._assert_registry_matches_manifest`). When editing a task's inputs or outputs, update **both** the Python attributes and the YAML entry; regression tests: [`src/runner/app/tests/test_yaml_matches_task_declarations.py`](src/runner/app/tests/test_yaml_matches_task_declarations.py) and [`src/api/tests/test_recon_task_manifest_public.py`](src/api/tests/test_recon_task_manifest_public.py). Rule: [`runner-task-patterns.mdc`](.cursor/rules/runner-task-patterns.mdc).
