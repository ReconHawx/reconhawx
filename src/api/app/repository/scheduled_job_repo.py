...
class ScheduledJobRepository:
    ...
    async def create_scheduled_job(job_data: Dict[str, Any]) -> bool:
        ...
        try:
            async with get_db_session() as db:
                ...
                # Convert program_id to UUID if it's a string
                program_uuid = None
                if job_data.get("program_id"):
                    try:
                        if isinstance(job_data["program_id"], str):
                            program_uuid = uuid.UUID(job_data["program_id"])
                        else:
                            program_uuid = job_data["program_id"]
                    except ValueError:
                        logger.error(f"Invalid program_id format: {job_data['program_id']}")
                        return False
    ...
    async def get_scheduled_job(schedule_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with get_db_session() as db:
                scheduled_job = db.query(ScheduledJob).outerjoin(Program, ScheduledJob.program_id == Program.id).filter(ScheduledJob.schedule_id == schedule_id).first()
                
                if not scheduled_job:
                    return None
                
                job_dict = scheduled_job.to_dict()
                # Add program name to the result
                job_dict['program_name'] = scheduled_job.program.name if scheduled_job.program else None
                return job_dict
    ...
    async def get_all_scheduled_jobs(user_id: Optional[str] = None, program_ids: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            async with get_db_session() as db:
                ...
                # Filter by program permissions if provided
                if program_ids:
                    try:
                        program_uuids = []
                        ...
                        query = query.filter(ScheduledJob.program_id.in_(program_uuids))
                    except ValueError as e:
                        logger.error(f"Invalid program_id format: {e}")
                        return []
                
                # Join with programs table to get program names
                query = query.outerjoin(Program, ScheduledJob.program_id == Program.id)
                ...
                for job in scheduled_jobs:
                    ...
                    # Add program name to the result
                    job_dict['program_name'] = job.program.name if job.program else None
        ...
        try:
            async with get_db_session() as db:
                ...
                if "workflow_variables" in update_data:
                    # Serialize workflow variables to handle datetime objects
