...
Superusers can trigger an in-cluster upgrade from **Admin → System upgrade** (`/admin/system-upgrade`): the API creates a one-shot **Job** using **`upgrader-sa`** that applies `kubernetes/base-update/` (same flow as [`update-kubernetes.sh`](update-kubernetes.sh)) and rolls app Deployments. Image: `UPGRADER_IMAGE` on the API pod (pinned with other services via `components/pinned-releases`). The Job entry uses **`bash -c`** (not a login shell); when a pod token is present, `upgrade.sh` clears merged kubeconfig env, writes a **minimal kubeconfig** aimed at the in-cluster apiserver, and sets **`KUBECONFIG`** to it (Alpine’s **`kubectl`** package may ship **`~/.kube/config`** with **`localhost:8080`**). The upgrader image removes **`/root/.kube`** after install. **RBAC escalation:** keep [`kubernetes/base/upgrader/upgrader-rbac.yaml`](kubernetes/base/upgrader/upgrader-rbac.yaml) in sync with namespaced Roles in **`base-update`** (e.g. **`job-manager`**, **`runner-role`**)—Kubernetes rejects applying rules the upgrader does not already hold. Air-gapped: stage a release tarball, then create the Job with `staging_id`. See [`docs/update-reconhawx.md`](docs/update-reconhawx.md) for when to use the UI vs. CLI scripts.
