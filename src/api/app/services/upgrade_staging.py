"""Ephemeral on-disk staging for upgrade tarballs (consumed by the upgrade Kubernetes Job)."""
...
STAGING_TTL_SEC = int(os.getenv("UPGRADE_STAGING_TTL_SEC", "86400"))
...
def _purge_stale_locked() -> None:
    ...
    for sid, sf in list(_staging.items()):
        if now - sf.created > STAGING_TTL_SEC:
            ...
            del _staging[sid]
            logger.info("removed stale upgrade staging id=%s", sid)
...
def _unlink_safe(path: str) -> None:
    try:
        ...
        logger.warning("could not unlink upgrade staging file %s: %s", path, e)
...
def register_file(abs_path: str) -> tuple[str, str]:
    """Register a staged tarball path. Returns (staging_id, pull_token)."""
    purge_stale()
